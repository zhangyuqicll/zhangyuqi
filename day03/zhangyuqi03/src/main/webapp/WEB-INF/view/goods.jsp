...
<body class="gray-bg">
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveGoods.htm">
						<input type="hidden" name="gId" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
								...
								<td colspan="4">
									<select class="form-control" name="tId" id = "tId" aria-required="true" required>
										...
										<c:forEach items="${tlist }" var="r">
										 	<option value="${r.tId }">${r.tTypename }</option>
										</c:forEach>
				                	</select>
								</td>
							</tr>
							<tr>
								<td>生产日期：</td>
								<td><input type="date" id="creatatime" name="creatatime"
									class="form-control" aria-required="true" required/></td>
							</tr>
						</table>
	...
	function initEditUser(getSelection){
		$('#hidden_txt_userid').val(getSelection.gId);
		$('#name').val(getSelection.name);
		$('#price').val(getSelection.price);
		$('#tId').val(getSelection.tId);
		$('#creatatime').val(new Date(getSelection.creatatime).Format("yyyy-MM-dd"))
	}
