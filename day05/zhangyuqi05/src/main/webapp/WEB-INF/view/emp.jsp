...
<body class="gray-bg">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="dDepid" id = "txt_search_roleid">
						<option value="0">---请选择---</option>
						<c:forEach items="${dlist }" var="r">
						 	<option value="${r.dDepid }">${r.dDname }</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveEmp.htm" enctype="multipart/form-data">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>性别：</td>
								<td>
								<input type="radio" id="gender" name="gender" aria-required="true" required value="男"/>男
								<input type="radio" id="gender" name="gender" aria-required="true" required value="女"/>女
							...
							<tr>
								...
								<td>
								<img alt="" src="" id="pics" width="50px" height="50px">
								...
								<td>爱好：</td>
								<td>
								<input type="checkbox" id="hobby1" name="hobby"  aria-required="true" required value="篮球"/>篮球
								<input type="checkbox" id="hobby2" name="hobby"  aria-required="true" required value="足球"/>足球
								<input type="checkbox" id="hobby3" name="hobby"  aria-required="true" required value="排球"/>排球
								</td>
	        ...
	        $('#table_user').bootstrapTable({
	           ...
	           // height: 500,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
	            ...
	            },
	            {
	                ...
	                title: '头像',
	                	formatter:function(value,row,index){
		                	return "<img src='/img/"+value+"' width='50px' height='50px'/>"
		                }
	            },{
	                field: 'dname',
	                title: '部门'
	...
	function initEditUser(getSelection){
		...
		$('input[value="'+getSelection.gender+'"]').prop("checked",true);
		$('#age').val(getSelection.age);
		$('#pics').attr('src',"/img/"+getSelection.pic);
		$('#pic').attr(getSelection.pic);
		$('#hobby1').prop("checked",getSelection.hobby.indexOf("篮球")!=-1);
		$('#hobby2').prop("checked",getSelection.hobby.indexOf("足球")!=-1);
		$('#hobby3').prop("checked",getSelection.hobby.indexOf("排球")!=-1);
		$('#dDepid').val(getSelection.dDepid);
