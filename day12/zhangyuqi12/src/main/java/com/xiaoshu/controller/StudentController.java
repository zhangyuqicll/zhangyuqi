...
public class StudentController extends LogController{
	...
	public String index(HttpServletRequest request,Integer menuid) throws Exception{
		...
		List<Major> clist=studentService.findMajor();
	...
	public void reserveUser(HttpServletRequest request,Student user,HttpServletResponse response){
		...
		try {
			if (userId != null) {   // userId不为空 说明是修改
				Student userName = studentService.existUserWithUserName(user.getSdname());
				...
				if(studentService.existUserWithUserName(user.getSdname())==null){  // 没有重复可以添加
	...
	@RequestMapping("ruStudent")
	public void ruStudent(MultipartFile ru, HttpServletRequest request,HttpServletResponse response){
		JSONObject result=new JSONObject();
		try {
			...
			for (int i = 1; i <= rowNum; i++) {
				...
				Integer mid=studentService.findById(cname);
				Student student = new Student();
				student.setMid(mid);
				student.setSdbirth(sdbirth);
				student.setSdhobby(sdhobby);
				student.setSdname(sdname);
				student.setSdsex(sdsex);
	...
	public void echartsStudent(Major major, HttpServletRequest request,HttpServletResponse response){
		JSONObject result=new JSONObject();
		try {
			List<StudentVo>elist=studentService.echartsStudent();
	...
	public void chuStudent(HttpServletRequest request,HttpServletResponse response){
		JSONObject result = new JSONObject();
		try {
			String time = TimeUtil.formatTime(new Date(), "yyyyMMddHHmmss");
		    String excelName = "手动备份"+time;
		   List<StudentVo>list= studentService.findPage(new StudentVo());
			String[] handers = {"编号","学生名字","学生性别","学生爱好","学生生日","专业"};
			// 1导入硬盘
		...
		try {
			...
			for (int i = 0;i < list.size(); i++) {
			   ...
			   if(!log.getSdhobby().contains("篮球")){
			    ...
			    row.createCell(4).setCellValue(TimeUtil.formatTime(log.getSdbirth(), "yyyy-MM-dd"));
